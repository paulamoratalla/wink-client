...
const ProfileFormEdit = () => {
    ...
    const { name, birth, identity, city, interestedGender, height, exercise, zodiac, education, drink, smoke, lookingFor, children, religion, political } = userData

    useEffect(() => {
        usersService
            .getOneUser(userId)
            .then(({ data }) => {
                setUserData({ name: data.name, birth: data.birth, identity: data.identity, city: data.city, interestedGender: data.interestedGender, height: data.height, exercise: data.exercise, zodiac: data.zodiac, education: data.education, drink: data.drink, smoke: data.smoke, lookingFor: data.lookingFor, children: data.children, religion: data.religion, political: data.political })
            })
    ...
    return (
        <Form onSubmit={handleSubmit}>
            ...
            <Button variant="dark" type="submit" style={{ width: '100%' }} onClick={handleSubmit}>Save Profile</Button>
